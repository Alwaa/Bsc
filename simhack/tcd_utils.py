...
def get_costs_sep(coil, msh, affine):
    tree = msh.get_AABBTree()
    subcost1 = lambda x: tree.any_point_inside(x)
    subcost2 = lambda x: np.mean(np.sqrt(tree.min_sqdist(x)))
    if 'intersectPoints' in coil['coilModel'][0]:
        cost1 = lambda x: subcost1(
            get_model_points(coil, x, fieldnames=['intersectPoints'], affine=affine)[0])
        cost2 = lambda x: subcost2(
            get_model_points(coil, x, fieldnames=['minDistancePoints'], affine=affine)[0])
    else:
        cost1 = lambda x: subcost1(
            get_model_points(coil, x, fieldnames=['points'], affine=affine)[0])
        cost2 = lambda x: subcost2(
            get_model_points(coil, x, fieldnames=['minDistancePoints'], affine=affine)[0])
        
    return cost1, cost2

def get_contraintf(coil, msh, affine):
    tree = msh.get_AABBTree()
    subcost2 = lambda x: np.mean(np.sqrt(tree.min_sqdist(x)))
    if 'intersectPoints' in coil['coilModel'][0]:
        cost2 = lambda x: subcost2(
...
def print_bounds(coil):
    bounds = []
    for deform in coil['deformationList']:
        bounds.extend(list((deform['range'])))
    print(bounds)
