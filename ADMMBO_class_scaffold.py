...
nun_samples = 301
point_per_axis_start = 6 # "-2" # 7 is original, 5 works for small feas area problem
#################################
...
min_proba, max_proba = -0.5,0.5

def gp_rc_ei(x,y,z,rho,M,gp_rc,hbest):
    z=np.atleast_2d(z)
    hh = hbest - 0.5*rho/M * np.sum((x[None]-z+y[None]/rho)**2,axis=1)
    theta = gp_rc.predict(z)
    theta[theta > max_proba] = max_proba
    theta[theta < min_proba] = min_proba
    theta = theta+0.5
...
def admmbo(cost, constraint, M, bounds, grid, x0, f0=None, c0=None,
    ...
    K3 = gaussian_process.kernels.ConstantKernel(constant_value_bounds=(1e-10,1e10)) * \
        gaussian_process.kernels.Matern(nu=1.5,length_scale_bounds=(1e-2, 1e2))
    K3 = gaussian_process.kernels.ConstantKernel(constant_value_bounds=(1e-10,1e10)) * \
        gaussian_process.kernels.Matern(nu=1.5,length_scale_bounds=(1, 1e1)) #lengthscale boud based on size of area??
    
    gp_rc = gaussian_process.GaussianProcessRegressor(kernel=K3)
    ...
    print(np.array(c0 - 0.5))
    gp_rc.fit(x0,np.array(c0 - 0.5))  
    ...
    while k < K and not S:
        ...
        for t in range(betac):
            #Line 4 in Alg. 3.3
            h = (gp_rc.y_train_ + 0.5) + \
                0.5*rho/M*np.sum((x[None]-gp_rc.X_train_
                                  +y[None]/rho)**2,axis=1)
            hbest = np.min(h)
            
            eif = lambda z: -gp_rc_ei(x,y,z,rho,M,gp_rc,hbest)
            qi = lambda zi: 0.5*rho/M * np.sum((x[None]-zi+y[None]/rho)**2,axis=1)
            hh = hbest - qi(grid)
            theta = gp_rc.predict(grid)
            theta[theta > max_proba] = max_proba
            theta[theta < min_proba] = min_proba
            theta = theta+0.5
            ...
            # print(ei.max())
            z = grid[np.argmax(ei)]
            gp_rc.fit(np.concatenate((gp_rc.X_train_,z[None]), axis=0),
                    np.concatenate((gp_rc.y_train_,constraint(z)-0.5),axis=0))
        h = (gp_rc.y_train_ + 0.5) + \
                0.5*rho/M*np.sum((x[None]-gp_rc.X_train_
                                  +y[None]/rho)**2,axis=1)
        z=gp_rc.X_train_[np.argmin(h)]
        print(f"{x}\n{z}\n{rho}")
        y += rho * (x - z)
        print(f'x: {x} \nz: {z} \ny: {y}')
        r = (x - z)**2
...
if __name__=='__main__':
    ...
    xo,zo,gpr,gp_rc=admmbo(costf, constraintf, M, bounds_array, grid, x0,alpha=2,beta=2,K=30)
    ...
    plt.figure();plt.imshow(gp_rc.predict(xy).reshape(len(xin),-1).T,
                            extent=(extent_tuple),origin='lower')
